...
![image9](assets/人评标准V2.1/image9.png)
...
![image10](assets/人评标准V2.1/image10.jpg)
...
| ***任务收敛效率*** | *在任务目标和当前阶段要求已经明确后，模型能否通过实现、验证和修正，使任务高效、稳定地达到可验收状态。重点观察首轮实现基础、每轮有效进展、根因定位、重复犯错、修改范围、回归和最终收敛；轮次仅作参考。* | *通用维度，不可选N/A* | *上限为5轮的：达5轮且最终无法修复得0分*<br>上限为3轮的（单轮修bug题目+多轮题）：3轮依然无法交付，得0分<br><br>如果是多轮题，就算每个轮次的平均debug轮次 | *上限为5轮的：4轮debug修复后完成得1.5分；4轮未debug完成达5轮但修复完成得1分*<br>上限为3轮的（单轮修bug题目+多轮题）：3轮可交付，得1分<br><br><br>如果是多轮题，就算每个轮次的平均debug轮次 | 上限为5轮的：用了2轮debug有效修复得2.5分；用了3轮debug修复得2分<br><br>上限为3轮的（单轮修bug题目+多轮题）：2轮能交付得2分；<br>如果是多轮题，就算每个轮次的平均debug轮次 | *上限为5轮的：首轮已形成正确主体，后续只需少量针对性修正，只需要1轮debug修复*<br>上限为3轮的（单轮修bug题目+多轮题）1轮能够交付，如果是多轮题，就算每个轮次的平均debug轮次（小于等于1轮） | *不需要debug，首轮即形成高质量实现，或遇到问题时能够主动验证、快速定位根因，并以最小必要改动完成修复；整个过程几乎没有无效尝试、重复错误和功能回归，以很低的人工协作成本达到高质量验收。* |
...
*在讨论质量之前，先统一 Rubric 的"数据结构"。我们建议（其他topic可以参考），每条 Rubric 包含以下字段：*

| ***字段*** | ***含义*** | ***取值*** | ***说明*** |
| --- | --- | --- | --- |
| ***Criterion（判定点描述）*** | *用自然语言描述的判定条件* | *自由文本* | 必须是Yes/No二元判定句（建议疑问句，可用陈述句），且 Yes = 得分 |
| ***Type（类型）*** | *客观可量化 vs 主观不可量化* | *Objective / Subjective* | 可以客观判断的 和 需主观判断的（审美居多） |
| ***Necessity（必要性）*** | *显性要求 vs 隐性要求* | *Explicit / Implicit* | *Explicit = 题目（prompt+历史上文等）明确写了的要求；Implicit = 需要推理符合具体实现的要求* |
| ***Priority（优先级）*** | *核心底线 vs 锦上添花* | *Must have / Nice to have* | / |
| ***Dimension（评测维度）（可多选）*** | *Rubric 对应的能力维度* | *见下方维度分类* | *可 by rubric维度聚合分析模型表现* |

### Dimension 维度分类

| ***评估维度*** | **特殊说明** |
| --- | --- |
| ***指令遵循*** | *是否满足题目需求指令的约束，prompt中明确禁止的约束需归档为指令遵循* |
| **功能完整性** | 功能、核心链路 |
| **视觉审美** | **非必选** |

**注意：每轮rubric总条数15-30条最好（如题目复杂可超出30条），每个dimension维度对应criterion至少5条**
...
| *层* | *输出* |
| --- | --- |
| *判定层* | 每条 criterion判断0/1，单条判定只允许二值（通过 / 不通过），不给部分分； |
| *计分层* | ***由通过条数计算维度分，取值 0 ~ 4.0，步长 0.5***<br>*n      = 该维度总条数*<br>*k      = 判定通过的条数*<br>*通过率 = k / n*<br>*原始分 = 通过率 × 4*<br>*维度得分 = 向下取整原始分* |

### 计算示例

| *维度* | *n* | *k* | *通过率* | *原始分* | *向下取整得维度得分* |
| --- | --- | --- | --- | --- | --- |
| *视觉审美* | *8* | *4* | *4/8 = 0.500* | 0.5 × 4 = 2.0 | ***2.0*** |
| *功能完整性* | *25* | *19* | *19/25 = 0.760* | 0.76 × 4 = 3.04 | ***3.0*** |

## 六、Vibe Tags 与备注（人评最重要的环节 ⭐）
...
![image11](assets/人评标准V2.1/image11.png)
